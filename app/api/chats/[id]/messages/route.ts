...
// Detección aproximada de tema por palabras (misma heurística que /api/chat)
...
const RESPUESTAS_INICIO = [
  (t: string) => `Sobre ${t || 'este tema'}, te lo cuento en corto.`,
  (t: string) => `Vale, tomemos ${t || 'esto'} como foco y bajemoslo a lo esencial.`,
  (t: string) => `Buen punto con ${t || 'esto'}. Te doy una guia rapida.`,
  (t: string) => `Hablemos de ${t || 'esto'} con ejemplos simples.`,
  (t: string) => `Para aprender ${t || 'esto'}, pensemos en ideas clave.`,
  (t: string) => `Vamos directo: ${t || 'este tema'} se entiende mejor con pasos cortos.`,
...
export async function POST(request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    if (!chat) return NextResponse.json<ChatMessageResponse>({ success: false, error: 'NOT_FOUND', message: 'Chat no encontrado' }, { status: 404 })
    if (chat.estado !== 'abierto') return NextResponse.json<ChatMessageResponse>({ success: false, error: 'INVALID_STATE', message: 'Chat cerrado' }, { status: 400 })
